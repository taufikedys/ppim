...
import csv, json, time, pymysql, folium
...
import re, matplotlib.pyplot as plt, community, os#, spacy
...
def searchWeb(keywords, start_= '', end_='', site='', N = 100, lan = ''):
    #keywords = 'PT TigaRaksa Indonesia'
    qry = [keywords]
    for par, syn in zip([site, start_, end_], ['site:', 'before:', 'after:']):
        if par:
            qry.append(syn+par)
    qry = ' '.join(qry)
    data = []
    headers = getHeaders()
    results = scrape_google(qry, N, lan, headers = headers)
    for result in results:
        data.append(result)
    if data:
        return pd.DataFrame(data)
    else:
        print('Query return zero results')
        return None
    
    
def getOldPost(keywords, start_, end_):
    qry = urllib.parse.quote(keywords.lower().strip())
    url = 'https://twitter.com/search?l=&q={}%20since%3A{}%20until%3A{}&src=typd'.format(qry, start_, end_)
    headers = {}
    ...
    for t in Tweets:
        ...
        posts.append({'time':wkt, 'username':usr, 'tweet':tweet, 'link':url, 'location':'','lat':0.0,'lon':0.0})
...
def getTweets(twitter, topic, N = 300, lan = None):
